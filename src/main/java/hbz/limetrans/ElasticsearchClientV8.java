...
public class ElasticsearchClientV8 extends ElasticsearchClient { // checkstyle-disable-line ClassDataAbstractionCoupling|ClassFanOutComplexity
    ...
    protected void getIndexes(final String aIndex, final BiConsumer<String, Boolean> aConsumer) {
        accept(c -> c.indices().get(b -> b.index(aIndex).features(Feature.Aliases))
                .result().forEach((i, s) -> aConsumer.accept(i, !s.aliases().isEmpty())));
    }
    ...
    private void getAliases(final Function<GetAliasRequest.Builder, ObjectBuilder<GetAliasRequest>> aFunction, final BiConsumer<String, IndexAliases> aConsumer) {
        accept(c -> {
            ...
            response.result().forEach(aConsumer);
        });
    }
